...
`ruoyi-6x/script/sql/apply-sql.sh` 执行脚本时按文件名排序。若使用 bash 默认 glob 顺序，`v10`、`v11`、`v12` 会排在 `v2~v9` 之前（字典序），导致依赖关系错乱。

**示例**：`procurement_6x_v12.sql` 在 `invoice_info` 上加 `matched_items`（位于 `ocr_json` 之后），而 `ocr_json` 由 `procurement_6x_v5_invoice_info.sql` 创建。若 v12 先于 v5 执行，会报：
```
ERROR 1054 (42S22): Unknown column 'ocr_json' in 'invoice_info'
```

**修复**：`apply-sql.sh` 已改用 `sort -V`（version sort），按数字大小排序。后续新增脚本继续按 `v<数字>_描述.sql` 命名即可。

### 8.7 applied-sql.log 与数据库状态一致性

如果曾经手动执行过 SQL 或脚本失败后未回滚 log，`applied-sql.log` 可能与数据库实际状态不一致。表现为：
- 某脚本已在数据库生效，但 log 里没记录，导致重复执行时报错。
- 某脚本执行失败但 log 里已有记录，导致跳过后续依赖脚本。

**排查**：
```bash
# 查看 log
tail -30 /home/liyang/jvkit/oa/sql/applied-sql.log

# 核对关键表结构
docker exec ruoyi-mysql mysql -uroot -pruoyi123 -D ry-vue-6x -e "DESC invoice_info"
```

**处理**：按数据库实际状态补齐/修正 log，确保二者一致后再运行 `deploy.sh`。
